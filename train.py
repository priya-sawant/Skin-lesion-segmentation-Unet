...
def read_mask(path):
    path = path.decode()
    x = cv2.imread(path, cv2.IMREAD_GRAYSCALE)  ## (H, W) to read img as grayscale
    x = cv2.resize(x, (W, H))
    x = x/255.0
    x = x.astype(np.float32)                    ## (256, 256)
    x = np.expand_dims(x, axis=-1)              ## (256, 256, 1) here we added channel
    return x
...
if __name__ == "__main__":
    ...
    """ Dataset : 60/20/20 """
    dataset_path = "C:/Users/Priyanka Sawant/Downloads/ISIC Challenge/"
    ...
    #to calculates batches- 1558//4=389
    ...
    callbacks = [
        ModelCheckpoint(model_path, verbose=1, save_best_only=True),            #to save weightfile while training
        ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=1e-7, verbose=1),
        CSVLogger(csv_path),                                                    #to log all the data while training
        TensorBoard(),                                                          #for visualization
        EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=False) #to reduce overfitting
    ]
